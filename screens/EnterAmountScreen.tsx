...
import { sanitizeDigitsDash } from '../src/security/sanitize'; // ✅ 추가
...
export default function EnterAmountScreen() {
  ...
  const onKey = (k: string) => {
    ...
    if (!/^\d+$/.test(k)) return; // '00','0' 같은 문자열은 숫자여도 개별 키 처리 대신 숫자만 허용
    const nextRaw = (amount + k).replace(/^0+(?=\d)/, ''); // 선행 0 정리
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (res.status === 403) {
        ...
        Alert.alert('송금 차단', typeof message === 'string' ? message : '정책에 의해 차단되었습니다.');
        return;
      }
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1 }} keyboardShouldPersistTaps="handled">
        <Container>
          ...
          <AccountInput
            ...
            value={counter}
            onChangeText={(t) => setCounter(sanitizeDigitsDash(t, 20))} // ✅ 적용
